...
if __name__ == "__main__":
    ...
    for models_struct_directory in tqdm(models_struct_directories):
        ...
        filename = filenames[2]  # only on test set to assess model sensitivity
        ...
        for epoch_num in range(num_epochs):
            ...
            Power = 0
            Powers = []
            # ranges = np.arange(0.005, threshold, 0.05)
            ranges = np.arange(10, 10000, 1)
            ...
            for N in tqdm(ranges):
                ...
                # print(df_preds["predictions_year" + str(1 + epoch_num)].shape)

                # cond_lhs = (
                #     (df_preds["predictions_year" + str(1 + epoch_num)] - threshold) < N
                # ) & (
                #     (df_preds["predictions_year" + str(1 + epoch_num)] - threshold) > 0
                # )
                # cond_rhs = (
                #     (df_preds["predictions_year" + str(1 + epoch_num)] - threshold) > -N
                # ) & (
                #     (df_preds["predictions_year" + str(1 + epoch_num)] - threshold) < 0
                # )
                # df_preds_N_closest = df_preds.loc[cond_lhs | cond_rhs].reset_index(
                #     drop=True
                # )
                ...
                #         chi2_var_test(
                #             s1, s2, bonferroni_corr=len(cols_to_test) * num_epochs ** 2
                #         )
                #     )
                # #################
                ...
                FN = np.sum(
                    ...
                    & (
                        df_preds_N_closest[
                            "year" + str(1 + epoch_num) + "_reduction_40_ge"
                        ]
                        == "S_1"
                    )
                )
            ...
            # print(np.sum(cond_lhs | cond_rhs))
            ...
            for var_to_assess in vars_to_assess:
                ...
                if epoch_num == 0:  ## year zero use study entry
                    aceiarb_var = "study_entry_aceiarb_flag"
                else:  # other years use year1-5
                    aceiarb_var = "year" + str(epoch_num) + "_aceiarb_coverage"
                aceiarb_beliefs = cure_ckd_bn.net.get_node_value(aceiarb_var)
                aceiarb_categories = [
                    cure_ckd_bn.net.get_outcome_id(aceiarb_var, i)
                    for i in range(len(aceiarb_beliefs))
                ]
                ...
                for aceiarb_category in aceiarb_categories:
                    for category in categories:
                        df_preds_N_closest_cat = df_preds_N_closest.copy(deep=True)
                        df_preds_N_closest_cat[var_to_assess] = category
                        df_preds_N_closest_cat[aceiarb_var] = aceiarb_category
                ...
                sens_results["categories"] = [
                    (aceiarb_category, category)
                    for aceiarb_category in aceiarb_categories
                    for category in categories
                ]
                ...
                for i in range(len(predictions_list) - 1):
                    for j in range(1, len(predictions_list)):
                        if (
                            i != j
                            and sens_results["categories"][i][0]
                            == sens_results["categories"][j][0]
                        ):  # same aceiarb category
                            combination = {}
                            combination["name"] = [
        ...
        dumpObjects(
            sens_res_epochs,
            "Data/genie_datasets/DBN_predictions/Results/sens_analysis_results/aceiarb_"
            + model_name
            + "_sens_analysis_results",
        )

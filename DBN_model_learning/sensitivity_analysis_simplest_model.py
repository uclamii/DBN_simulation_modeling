    ...
    for models_struct_directory in tqdm(models_struct_directories):
        ...
        for epoch_num in range(num_epochs):
            ...
            for N in tqdm(ranges):
                ...
                ####################################################
                # # target variable - when teesting every single variable
                ...
                if np.any(covariates_p_vals): # if any test is significant
            ...
            for var_to_assess in vars_to_assess:
                ...
                for category in categories:
                    ...
                    predictions_list.append(np.round(predictions, 9))  # round up prob
                ...
                for i in range(len(predictions_list) - 1):
                    for j in range(1, len(predictions_list)):
                        if i != j:
                            ...
                            print(
                                "Average causal effect of each category: ",
                                np.mean(predictions_list[i]),
                                np.mean(predictions_list[j]),
                            )
                        else:
        ...
        dumpObjects(
            ...
            + model_name
            + "_sens_analysis_results_applied_on_"
            + testing_site,
        )

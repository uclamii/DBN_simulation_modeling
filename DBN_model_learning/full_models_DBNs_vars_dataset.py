...
def create_model_and_evaluate(models_struct_directory, structures_path, NUM_EPOCHS):
    """
    Method used to parallelize models execution and evaluation. Can also be run in a for loop sequentially.

    """
    parallelize_inference = False
    ...
    model_intraDBN_struct = [
        struct for struct in models_struct_directory_structs if "Intra_DBN" in struct
    ][0]
    model_Contemporals_struct = [
        struct for struct in models_struct_directory_structs if "Contemporals" in struct
    ][0]
    ...
    # set BN_static_t0=False to use the structure method that creates the
    # remaining epochs.
    ...
    cure_ckd_bn.create_BN(
        ...
        BN_static_t0=False,  # false to create DBN
        num_epochs=NUM_EPOCHS,
    )

    # train network
    # cure_ckd_bn.trainBN(df_train=df_train, model_name=model_name)

    # learn network structure not necessary when using RAUS
    ...
    for filename in filenames:
        ...
        # test network
        # adjust number of epochs bases on horizon length
        ...
        for epoch_num in range(0, num_of_epochs):
            ...
            if parallelize_inference:
                predictions = parallelTestBN(
                    model_name,
                    df_valid,
                    cols_to_test,
                    target,
                ...
                predictions = cure_ckd_bn.parallelTestBN(
                    df_valid,
                    cols_to_test,
                    target,
                    ignore_warnings=False,
...
if __name__ == "__main__":
    ...
    NUM_EPOCHS = 6
    print(models_struct_directories)
    ...
    results = Parallel(n_jobs=1)(
        (
            delayed(create_model_and_evaluate)(
                models_struct_directory, structures_path, NUM_EPOCHS
            )
        )
        for models_struct_directory in tqdm(models_struct_directories)
    )

function bnet = learn_params_dbn2(bnet, data)
...
for j=1:ss
  ...
  if adjustable_CPD(bnet.CPD{e})
    ...
    bnet.CPD{j} = learn_params(bnet.CPD{e}, fam, data, bnet.node_sizes, bnet.cnodes); 
% F.Denk 01.07.2017 - learn_params input parameters in previous script did not match the required ones
  end
end
...
%% F.Denk 01.07.2017 family of nodes in slice 2 are same as for nodes in slice 1 except j2 = j + ss -- equivalence class ensures correct use of parameters
    bnet.CPD{e} = learn_params(bnet.CPD{e}, fam, data2, bnet.node_sizes, bnet.cnodes); 
% F.Denk 01.07.2017 - learn_params input parameters in previous script did not match the required ones
  end
end

...
def df2Orange(df, class_name):
    ...
    domain = Orange.data.Domain(
        [ContinuousVariable(col) for col in cols], class_vars=class_var
    )

    data = Orange.data.Table.from_list(domain, df.values.tolist())
...
def dict2list(d):
    ...
    for i, val in enumerate(d.values()):
        ...
        elif i == len(d.values()) - 1:
            bins.append(float(val.replace(" ", "").replace("≥", "")))
        elif "-" in val:
            bin1, bin2 = val.split(" - ")  # .replace(" ", "")
    ...
    def transform(self, df, mapped=True, class_name="target"):
        ...
        for col in cols:
            ...
            if mapped:
                ...
                # labels = list(range(len(bins) - 1)) # counter start at 0
                labels = list(range(1, 1 + len(bins) - 1))  # counter start at 1
...
if __name__ == "__main__":
    ...
    print(discritizer.transform(df=iris)[:3])
